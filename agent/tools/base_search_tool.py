...
class BaseSearchInput(BaseModel):
    ...
    min_similarity: float = Field(
        default=0.7, description="Минимальное сходство для результатов"
    )
...
class BaseSearchTool(BaseTool):
    ...
    def _cache_results(
        self, cache_key: str, results: List[Dict[str, Any]], ttl_seconds: int = 3600
    ):
    ...
    def _run(
        ...
        **kwargs: Any,  # Принимаем доп. аргументы для ключа кеша
        ...
        raise NotImplementedError(
            "Метод _run должен быть реализован в дочерних классах"
        )
